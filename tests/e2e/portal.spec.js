...
const ADMIN_PRIVATE_KEY_INVALID_ALG = ADMIN_PRIVATE_KEY.replace("\"alg\":\"RSA-OAEP-256\"", "\"alg\":\"RSA-OAEP-384\"");
  ...
  await page.evaluate(() => {
    try {
      ...
      if (typeof sessionStorage !== "undefined") {
        sessionStorage.removeItem("oreoremcp.adminSession");
      }
    } catch {
...
test("admin unlock rejects non-@github key algorithm", async ({ page }) => {
  ...
  await page.locator("#operator-account").fill(ADMIN_OPERATOR);
  await page.locator("#admin-passphrase").fill("oreoremcp-admin");
  await page.locator("#admin-private-key").fill(ADMIN_PRIVATE_KEY_INVALID_ALG);
  await page.locator("#admin-unlock").click();
  await expect(page.locator("#admin-status")).toContainText("Admin unlock failed");
});

test("admin unlock requires private key body", async ({ page }) => {
  await clearLocalStorageState(page);
  await page.goto("/");
  await page.locator("#operator-account").fill(ADMIN_OPERATOR);
  await page.locator("#admin-passphrase").fill("oreoremcp-admin");
...
test("tutorial and quick check are visible to users", async ({ page }) => {
  ...
  await expect(page.locator("#user-self-check")).toBeVisible();
  await expect(page.locator("#quick-check-blocked")).toBeVisible();
  await expect(page.locator("#quick-check-short-pass")).toBeVisible();
  await expect(page.locator("#quick-check-github-key")).toBeVisible();
  await expect(page.locator("#quick-check-run-works")).toBeVisible();
});
